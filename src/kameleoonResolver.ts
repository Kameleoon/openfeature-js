...
import { DataType, ResolveParams, Resolver } from '@kameleoon/openfeature-core';
...
export class KameleoonResolver implements Resolver {
  ...
  private readonly visitorCode: string;

  constructor(client: KameleoonClient, visitorCode: string) {
    this.client = client;
  ...
  resolve<T>({
    ...
    try {
      ...
      if (!isAnyType && typeof defaultValue !== typeof value) {
        return this.makeResolutionDetails({
          value: defaultValue,
          variant,
